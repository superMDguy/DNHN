...
def run_app(args):
    ...
    try:
        print(
            'Executing python3 -m farg.apps.{0}.run_{1} {2}'.format(appName, appName, rest))
        os.system(
            'python3 -m farg.apps.{0}.run_{1} {2}'.format(appName, appName, rest))
...
def addCodelet(args):
    ...
    codeletPath = os.path.join(
        os.getcwd(), 'farg', 'apps', appName.lower(), 'codelet_families', 'all.py')
...
def remove(args):
    appName = args.app_name
    yOrN = input(
        "Are you sure you want to remove {}? This cannot be undone. [y/n] ".format(appName))
    dirToRemove = os.path.join(
        os.getcwd(), 'farg', 'apps', appName.lower().replace(" ", ""))
    if yOrN[0].lower() == "y":
        dirToRemove = os.path.join(
            os.getcwd(), 'farg', 'apps', appName.lower())
...
def main():
    ...
    subparsers = parser.add_subparsers(
        help="For detailed help, try --help after command (e.g., run --help)")

    parser_run = subparsers.add_parser(
        'run', help='Run an app. For detailed help, add --help after app name (e.g., run seqsee --help)')
    parser_run.add_argument(
        'app_name', help='Name of app to create (all lowercase)')
    parser_run.add_argument('rest', nargs=argparse.REMAINDER,
                            help='Remaining arguments to pass on to app')
    parser_run.set_defaults(func=run_app)

    parser_create = subparsers.add_parser(
        'create', help='Create skeleton for a new app')
    parser_create.add_argument(
        'app_name', help='Name of app to create (all lowercase)')
    parser_create.set_defaults(func=create)

    parser_create_codelet = subparsers.add_parser(
        'codelet', help='Add a codelet family')
    parser_create_codelet.add_argument(
        'app_name', help='Name of app to create (all lowercase)')
    parser_create_codelet.add_argument(
        'codelet_name', help='Codelet family to create (without CF_prefix)')
    ...
    parser_remove.add_argument(
        'app_name', help='Name of app to remove (all lowercase)')
    parser_remove.set_defaults(func=remove)

    parser_test = subparsers.add_parser('test', help='Run tests')
    parser_test.add_argument(
        'app_name', help='Name of app to test. Use "core" for core tests')
    parser_test.set_defaults(func=run_tests)

    parser_update = subparsers.add_parser(
        'update', help='Updates farg to latest version using git')
