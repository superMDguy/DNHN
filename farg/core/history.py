...
class History(object):
    ...
    def AddArtefact(cls, item, artefact_type, log_msg, parents=None):
        ...
        item._hid = hid
        event_details = dict(eid=eid, type=EventType.CREATE,
                             hid=hid, artefact_type=artefact_type)
    ...
    def AddEvent(cls, event_type, log_msg, item_msg_list):
        ...
        event_details = dict(eid=eid, type=event_type,
                             log=log_msg, objects=item_msg_list_with_id)
        cls._event_log.append(event_details)
...
class HistoryGUI(object):
    ...
    def _AddSummaryFrame(self):
        ...
        self.summaryText = Text(summaryFrame)
        self.summaryText.pack()
        self.historyNB.add(summaryFrame, text='Summary',
    ...
    def _AddCountsFrame(self):
        ...
        self.countsText = Text(countsFrame)
        self.countsText.pack()
        self.historyNB.add(countsFrame, text='Counters',
                           underline=0, padding=2)
    ...
    def _AddObjectHistoryFrame(self):
        ...
        objHistoryLbl.pack()
        self.historyNB.add(
            objHistFrame, text="Object History", underline=0, padding=2)
...
<<<<<<< HEAD
        ...
        submit.pack()
        eventsLbl.pack()
        self.historyNB.add(eventsFrame, text="Object Events",
                           underline=0, padding=2)
    ...
    def GroupObjectEventsByClass(cls, hid):
        ...
        for event in events:
            if event['type'] is EventType.OBJECT_FOCUS:
                ...
                hid_here = event['hid']
                groupedObjects[
                    'CREATE ' + History._object_details[hid_here]['class_name']].append(event['eid'])
            else:
    ...
    def Summary(cls):
        summaryStr = ""
        obj_by_cls = sorted(cls.GroupObjectsByClass().items(),
                            reverse=True, key=lambda x: len(x[1]))
        for objClass, eventIds in obj_by_cls:
            summaryStr += "\t%5d\t%s" % (len(eventIds), objClass) + "\n"
            summaryStr += "\t\t" + '; '.join(str(eid)
                                             for eid in eventIds[:10]) + "\n"
        return summaryStr
    ...
    def EventsForItem(cls, hid):
        eventsStr = ""
        hid = int(hid)
        obj_by_cls = sorted(cls.GroupObjectEventsByClass(
            hid).items(), reverse=True, key=lambda x: len(x[1]))
        for objClass, eventIds in obj_by_cls:
            eventsStr += "\t%5d\t%s" % (len(eventIds), objClass) + "\n"
            eventsStr += "\t\t" + '; '.join(str(eid)
                                            for eid in eventIds[:10]) + "\n"
        return eventsStr
    ...
    def ObjectHistory(cls, hid, print_depth=0, max_depth=5):
        ...
        objHistoryStr += '*    ' * \
            print_depth, '[%d]\t%s\t%s' % (
                hid, details['class_name'], details['log']) + "\n"
        if print_depth >= max_depth:
            return
        if 'parents' in details:
            for parent in details['parents']:
                cls.ObjectHistory(
                    parent, print_depth=print_depth + 1, max_depth=max_depth)
=======
    Returns:
      A dictionary with cls as key and list of object indices as value.
    """
