...
class Main:
    ...
    # Not a constant as thought by pylint: disable=C6409
    run_mode_gui_class = gui.RunModeGUI
    #: Class to use for running in Batch mode.
    # Not a constant pylint: disable=C6409
    ...
    def _VerifyLTMPath(self):
        ...
        if self.flags.ltm_directory:
            if not os.path.exists(self.flags.ltm_directory):
                print("LTM directory '%s' does not exist." %
                      self.flags.ltm_directory)
                sys.exit(1)
        else:
            self._VerifyPersistentDirectoryPath()
            self.flags.ltm_directory = os.path.join(
                self.flags.persistent_directory, 'ltm')
            if not os.path.exists(self.flags.ltm_directory):
    ...
    def _VerifyStatsPath(self):
        ...
        if self.flags.stats_directory:
            ...
            self._VerifyPersistentDirectoryPath()
            self.flags.stats_directory = os.path.join(
                self.flags.persistent_directory, 'stats')
            if not os.path.exists(self.flags.stats_directory):
    ...
    def _CreateRunModeInstance(self):
        ...
        if run_mode_name == 'gui':
            ...
            input_spec = list(input_reader.ReadFile(
