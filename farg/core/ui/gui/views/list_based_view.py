...
class ListBasedView(ViewPort, metaclass=ABCMeta):
    ...
    def _DrawPreviousPageArrow(self):
        ...
        self.canvas.tag_bind(
            item_id, '<1>', lambda e: self._DecrementPageNumber())
    ...
    def _DrawNextPageArrow(self):
        ...
        item_id = self.canvas.create_polygon(x1, y1, x2, y2, x3, y3)
        self.canvas.tag_bind(
            item_id, '<1>', lambda e: self._IncrementPageNumber())
    ...
    def ReDraw(self, controller):
        ...
        if self.current_page_number > max_page_number:
            self.current_page_number = 1
        index_of_first_item = (
            self.current_page_number - 1) * self.items_per_page
        ...
        self.canvas.create_text(x, y, text=top_message,
                                anchor=NW, fill='#3333FF')

        if max_page_number > 0:
            x, y = self.CanvasCoordinates(self.width - 25, 0)
            self.canvas.create_text(x, y,
                                    text='p. %d/%d' % (self.current_page_number,
                                                       max_page_number),
