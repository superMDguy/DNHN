...
def GetTStatsDict(left_numbers, right_numbers):
    ...
    dof = n1 + n2 - 2
    svar = ((n1 - 1) * left_variance + (n2 - 1) *
            right_variance) / float(dof) if dof else 0
...
class RunStats:
    ...
    def AddData(self, data_string):
        ...
        if len(pieces) == 1:
            ...
            self.stats_per_state[pieces[0]].AddData(
                codelet_count=int(pieces[1]))
        self.count += 1
    ...
    def __str__(self):
        ...
        individual_strings = ('%s: %s' % (k, str(v))
                              for k, v in self.stats_per_state.items())
...
class AllStats:
    ...
    def IsRightBetter(self, for_input):
        ...
        """
        codelet_count_stats, success_stats = self.GetComparitiveStats(
            for_input)
