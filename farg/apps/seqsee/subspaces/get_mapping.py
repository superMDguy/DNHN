...
class CF_ExplainValues(CodeletFamily):
    ...
    def Run(cls, controller, *, me):
        ...
        logger.debug("Will love to explain this value: %s",
                     str(attribute_value))
        ...
        for attribute_to_use in WeightedShuffle(list(attributes_to_use.items())):
            ...
            if mapping:
                attribute_explanations[one_attribute] = (
                    attribute_to_use, mapping)
                if ws.category.AreAttributesSufficientToBuild(list(attribute_explanations.keys())):
                    ...
                    raise AnswerFoundException(
                        full_mapping, codelet_count=controller.steps_taken)
        controller.AddCodelet(family=CF_ExplainValues,
                              urgency=100, parents=[me])
