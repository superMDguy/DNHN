...
class SAnchored(LTMStorableMixin, FocusableMixin):
    ...
    def GetLTMStorableContent(self):
        ...
        return LTMStorableSObject(structure=structure, vector=self.object.vector)
    ...
    def GetAffordances(self, controller):
        ...
        for relation in self.relations:
            ...
            # on its confidence, whether it is internal (part of a group), and
            # other considerations.
            codelets.append(Codelet(CF_FocusOn, controller,
                                    25, dict(focusable=relation)))
        ...
        for edge in outgoing_isa_edges:
            # QUALITY TODO(Feb 10, 2012): Category activation and edge strength
            # relevant here.
    ...
    def GetSimilarityAffordances(self, other, other_fringe, my_fringe, controller):
        logging.debug(
            'GetSimilarityAffordances called: [%s] and [%s] ', self, other)
        ...
        if left.end_pos >= right.start_pos:
            ...
            # So both are anchored, and have overlapping fringes. Time for
            # subspaces!
