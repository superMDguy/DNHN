...
class WorkspaceView(ViewPort):
    ...
    def ReDraw(self, controller):
        ...
        groups = sorted(workspace.groups, reverse=True,
                        key=lambda x: x.end_pos - x.start_pos)
        for group in groups:
            start, end = group.start_pos, group.end_pos
            x1, y = self.CanvasCoordinates(
                start * space_per_element, self.elements_y)
            x2, y = self.CanvasCoordinates(
                (end + 1) * space_per_element, self.elements_y)
            span = 1 + end - start
