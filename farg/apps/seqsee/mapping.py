...
class StructuralMapping(Mapping):
    ...
    def __str__(self):
        pieces = 'Structural: [%s] %s' % (
            self.category.BriefLabel(), self.bindings_mapping)
        if self.slippages:
    ...
    def Apply(self, item):
        ...
        for attribute, v in self.bindings_mapping:
            new_binding_for_attribute = v.Apply(
                bindings.GetBindingsForAttribute(attribute))
...
def FindMapping(o1, o2, *, controller, seqsee_ltm, category=None):
    ...
    if category:
        ...
        if common_categories:
            category = SelectWeightedByActivation(
                seqsee_ltm, common_categories)
            return category.FindMapping(o1, o2, controller=controller, seqsee_ltm=seqsee_ltm)
        else:
            return None
